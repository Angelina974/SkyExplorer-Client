kiss.app.defineModel({
    ...
    methods: {
        ...
        async checkStatus(applicationId) {
            ...
            if (!this.getStatus()) {
                return app.msg.featureLocked(txtTitleCase("inactive account"), txtTitleCase("#account locked"), false)
            }

            // Account is active: check if the user is an active user
            if (!this.isActiveUser(kiss.session.userId)) {
                return app.msg.featureLocked(txtTitleCase("inactive user"), `${txtTitleCase("#user locked")}`, false)
            }
            ...
            if (applicationId) {
                ...
                if (!activeApplicationIds.includes(applicationId)) {
                    app.msg.featureLocked(txtTitleCase("inactive application"), `${txtTitleCase("#application locked")}`, false)
                }
            }
        },
